    ...
    for j in range(1, len(jL[vm[i]]), 1):
        if jL[vm[i]][j] in dag:
            ...
            for k in range(0, len(dag[jL[vm[i]][j]]), 1):
                if temp < commcost(jL[vm[i]][j], dag[jL[vm[i]][j]][k]
                        , result[jL[vm[i]][j]]['vm'], result[dag[jL[vm[i]][j]][k]]['vm']):
                    temp = commcost(jL[vm[i]][j], dag[jL[vm[i]][j]][k]
                                    , result[jL[vm[i]][j]]['vm'], result[dag[jL[vm[i]][j]][k]]['vm'])
...
for i in range(0, len(temp), 1):
    total += temp[i]
print('Total instance hour(With DataCenter):', temp, "\n", "total:", total)
